...
def discover_config(script_name, h5_fnam, config_fnam='', config_dirs = ['process', 'bin']):
    import sys, os
    root = os.path.split(st.__file__)[0]
    
    # config output file is [h5_data directory]/[sript_name].ini
    #
    config_output = os.path.join(os.path.split(h5_fnam)[0], script_name + '.ini')
